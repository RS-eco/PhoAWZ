...
env_dat <- aggregate(env_dat, fact=3)
env_dat
#env_dat <- projectRaster(env_dat, crs="+proj=tmerc +lat_0=0 +lon_0=9 +k=0.9996 +x_0=32500000 +y_0=0 +ellps=GRS80 +units=m +no_defs")
...
quantiles <- (0:8)/8                          # how many quantiles we want to map 
colours <- scico(8, palette = 'roma')    # 7 evenly interpolated colors 
quantile.vals <- round(quantile(env_dat$Tiefe, quantiles, names=F), 0) # the values for each quantile
val.remap <- (quantile.vals - min(env_dat$Tiefe)) / 
  diff(range(env_dat$Tiefe))                                # The values corresponding to the quantiles
p1 <- ggplot() + 
  geom_sf(data = outline, fill = "#dddddd", colour="black", size=0.75/.pt) +
  geom_tile(data=env_dat, aes(x=x, y=y, fill=Tiefe)) + 
  coord_sf(xlim=c(-10,22), ylim=c(40,70), expand=c(0,0)) + 
  scale_fill_gradientn(
    colours=colours,
    values=val.remap,
    breaks=quantile.vals,# Necessary to get legend values spread appropriately
  ...
  theme_bw() + theme(axis.title = element_blank(),
                     legend.key.height = unit(0.5, 'cm')) #change legend key height
ggsave(filename="figures/depth_all.png", p1_out, width=8, height=9, dpi=600)
...
p2 <- ggplot() + 
  geom_sf(data = outline, fill = "#dddddd", colour="black", size=0.75/.pt) +
  geom_tile(data=env_dat, aes(x=x, y=y, fill=SSS)) + 
  coord_sf(xlim=c(-10,22), ylim=c(40,70), expand=c(0,0)) + 
  scale_fill_gradientn(
    colours=colours,
    values=val.remap,
...
p3 <- ggplot() + 
  geom_sf(data = outline, fill = "#dddddd", colour="black", size=0.75/.pt) +
  geom_tile(data=env_dat, aes(x=x, y=y, fill=SST)) + 
  coord_sf(xlim=c(-10,22), ylim=c(40,70), expand=c(0,0)) + 
  scale_fill_gradientn(
    colours=rev(colours),
    values=val.remap,
  ...
  theme_bw() + theme(axis.text.y = element_blank(),
                     ...
                     legend.key.height = unit(0.5, 'cm')) #change legend key height
p1 + p2 + p3
ggsave(filename="figures/env_all.png", width=10, height=4, dpi=1000)
